# Bull-HF-BTC Codex — high-frequency BTC trading agent (sim-only)
...
> **Distinct from Bull-equities and the Claude HF loop** — own instructions, own state, own Telegram header (`BTC-HF CODEX`). Same chat ID, no API keys.

## What runs each tick (15 min)

1. **`harness.py prepare`** — pulls Alpaca crypto OHLCV (1m/5m/15m/1h × 200 bars), computes 8 indicators (RSI, MACD, Bollinger, ATR, EMA20/50/200, VWAP, Volume z-score), gates a chart-img fetch (50/day quota), builds `/tmp/hf_codex_prompt.md` + `/tmp/hf_codex_context.json`.
2. **`codex exec -m gpt-5.5` non-interactive** — Codex receives the same prompt, uses web search for BTC news when enabled, and emits a strict JSON decision (`OPEN_LONG/OPEN_SHORT/CLOSE/HOLD/SKIP` + entry/TP/SL/sizing/confidence/reason_fr).
...
# 3. Dry-run (no Codex, fake SKIP decision)
...
sudo systemctl enable --now bull-hf-btc-codex
sudo journalctl -u bull-hf-btc-codex -f
```

The service expects:
- `/opt/thashi-ai/hf_btc_codex/` checkout
- `/opt/thashi-ai/.env` with secrets
- User `bull` + group `bull` with read access to the repo
- `codex` CLI on PATH (`/usr/bin/codex` by default)
...
~96 ticks/day × ~10-15k token GPT-5.5 input + ~2k output. Web search calls are enabled by default with `HF_CODEX_SEARCH=1`.
...
- **Time alignment** — every tick aligns to a 15-min UTC boundary. If a tick takes > 15 min (e.g. Codex slow), the next one fires immediately to catch up.
