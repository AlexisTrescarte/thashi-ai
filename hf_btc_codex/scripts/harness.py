#!/usr/bin/env python3
"""Bull-HF-BTC Codex harness — orchestrates each 15-min tick.

Sub-commands (called by run_loop.sh in order):

    prepare              build /tmp/hf_codex_prompt.md + /tmp/hf_codex_context.json
    post                 parse /tmp/hf_codex_decision_envelope.json (Codex output),
    ...
    tick_dry             prepare + fake HOLD decision + post (no Codex call) — for testing.
...
TMP_PREFIX = "hf_codex" if ROOT.name.endswith("_codex") else "hf"
PROMPT_FILE = Path(f"/tmp/{TMP_PREFIX}_prompt.md")
CONTEXT_FILE = Path(f"/tmp/{TMP_PREFIX}_context.json")
ENVELOPE_FILE = Path(f"/tmp/{TMP_PREFIX}_decision_envelope.json")
DECISION_FILE = Path(f"/tmp/{TMP_PREFIX}_decision.json")
...
def _build_prompt(ctx: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "Utilise la recherche web (1 appel) avec : `BTC bitcoin news {date} ETF flow funding sentiment`. Filtre la dernière fenêtre 6h. Synthétise en 3-5 puces. Tu DOIS faire cette recherche avant de décider.",
        "",
    ...
    if chart_path:
        lines += [
            ...
            f"`{chart_path}` (5m, 800×600, RSI+MACD+Volume). Lis-le si pertinent (la tendance et les niveaux y sont plus lisibles).",
            "",
        ]
    lines += [
        "## 4. Sim portfolio",
...
def cmd_post(force_decision: dict[str, Any] | None = None) -> int:
    if not CONTEXT_FILE.exists():
        print(f"post error: missing {CONTEXT_FILE}")
        return 1
...
def cmd_tick_dry() -> int:
    cmd_prepare()
    print(f"\n--- {PROMPT_FILE} ({len(PROMPT_FILE.read_text())} chars) ---")
