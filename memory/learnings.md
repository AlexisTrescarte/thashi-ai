...
### 2026-04-27T20:10:00Z — BASELINE-SET
**Context**: Run `market-close` du 27 avril 2026 (15:10 CT). Première routine `market-close` à effectivement poster un baseline depuis le démarrage de l'agent (les routines market-close du 04-20 et du 04-24 ont été manquées par le harness — voir `[INCIDENT] 2026-04-24` pour la cascade des 6 runs ratés). `portfolio.md` `Baseline` fields étaient en placeholder (`(first live run date)`). Les references SPY 714.05 / QQQ 664.06 capturées par le `daily-review` du 04-24 étaient ad-hoc (post-close du vendredi) et n'avaient pas été promues en baseline officielle. Equity du jour : $97,461.39 ; SPY mid $714.89 ; QQQ mid $663.82 ; bench blend $689.36.
**Takeaway**: Le baseline officiel est posé aujourd'hui 27/04 dans `portfolio.md` — toutes les futures comparaisons cumul perf vs benchmark partent de cette date. Le drift pre-baseline ($100,000 starting capital → $97,461.39 = -2.54% sur ~1 mois de paper trading silencieux) n'est pas comptabilisé dans l'alpha — il n'y avait pas de benchmark posté à comparer. ATH tracking également initialisé au niveau du baseline ($97,461.39, 04-27) pour permettre au mécanisme drawdown auto-defense -20% de fonctionner sur une référence stable. Si l'équité passe sous $77,969.11 (= $97,461.39 × 0.80), auto-defense déclenchée.
**Action**: (1) `memory/equities/portfolio.md` mis à jour : Latest snapshot block + Baseline block + ATH block + Open positions table (GOOGL seule) + Open risks. (2) `runs.log` recevra une ligne `market-close ok — baseline set $97,461.39, SPY $714.89, QQQ $663.82, bench $689.36`. (3) Telegram FR mandatoire envoyé. (4) Le futur weekly-review et monthly-review devront utiliser ce baseline 04-27 comme point de départ — éviter de le re-poster.
**Agent**: equities
