...
### 2026-04-24T08:12:15Z — API-DEGRADED
**Context**: Run `crypto-hourly` du 24 avril 2026 08:12 UTC (scan 08:00 UTC cycle). Premier appel `python scripts/alpaca_crypto_client.py account` → `Alpaca HTTP 503 on GET https://paper-api.alpaca.markets/v2/account: DNS cache overflow`. Retry #1 après 3s : même erreur 503 DNS cache overflow. Retry #2 après 8s : même erreur. Trois tentatives consécutives échouées sur le même endpoint.
**Takeaway**: L'endpoint paper Alpaca retourne une 503 persistante avec signature "DNS cache overflow" — signature d'un problème d'infra côté broker (resolver saturé), pas d'une erreur d'auth ou de rate-limit côté Bull. Sans accès `account` → impossible de vérifier equity/cash → impossible de vérifier positions → impossible d'exécuter le cycle crypto-hourly (détection auto-defense, regime pulse, gestion positions, BUY dispatch). Per commande crypto-hourly étape 2 : terminer proprement, ne rien inventer. Les trailing stops natifs Alpaca continuent de protéger le book indépendamment de ce run. Prochain run 12:00 UTC re-tentera ; si persistant, monter en DEGRADED répété et considérer escalade humaine.
**Action**: (1) Aucun appel trade/ordre émis. (2) Aucun research note loggé (pas de regime pulse fiable sans cross-check account). (3) Notification Telegram DEGRADED envoyée. (4) runs.log append via journal skill, commit no-op pour trace.
**Agent**: crypto
