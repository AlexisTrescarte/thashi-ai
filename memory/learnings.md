...
### 2026-04-28T13:43:30Z — LESSON
**Context**: Run `market-open` 04-28 (08:43 CT). BUY queue = 2 names (VRT Probe 2.5%, AMD Probe 2.5%) + 1 critical pre-stage (GOOGL pre-earnings exit). Preflight all green. Quotes at the open: VRT bid $291.65 / ask $309.62 → spread **5.81%** (cap 0.5%) → SKIP mechanical. AMD: my direct quote pull saw bid $321.19 / ask $321.52 (10 bps) but the microstructure-confirm subagent 30 seconds later saw bid $310.28 / ask $325.00 → spread **463 bps** vs 30 bps cap → BLOCK non-overridable for Probe sizing. Both BUYs aborted at the open. GOOGL pre-stage executed cleanly: cancelled native trail $324.93, placed MOC sell qty=7 GTD today (order 33e1dd7d) — position exchange-bound for tomorrow's earnings (04-29 AMC).
**Takeaway**: This is the **third recurrence in 6 trading days** of "early-open thin-book → spread cap forces SKIP" (04-23 GEV+VRT; 04-28 VRT+AMD again). The 04-23 lesson said "intraday-scan 10:30 once book has settled" — it remains the right behavior. The new datapoint today: AMD's spread oscillated between 10 bps and 463 bps in ~30 seconds at 08:41-08:43 CT, meaning the **single-quote sample at the moment of decision is unreliable** in the first 15 min — the microstructure subagent's BLOCK based on a wider sample saved a likely bad fill. Validates the design: pretrade-check is doing its job. Implication for the prompt-evolution backlog: consider raising the "BUY queue dispatch slot" to 09:00 CT (open + 30 min) for the next monthly review, OR explicitly tag pre-market BUYs as "open-first" vs "settle-first" to drive the dispatch slot per name. Queued for monthly-deep-review 04-30 (last Friday this month).
**Action**: (1) Re-queue VRT + AMD to intraday-scan 10:30 CT today (per research_log). (2) Skips logged as `trade.abort` events in `runs.jsonl` via `otel_log.py`. (3) GOOGL pre-stage done — exit will fire at close auction (16:00 ET). (4) Telegram FR notification sent (over-notification preferred). (5) No guardrail violation, no thesis misread — discipline working as intended.
**Agent**: equities
