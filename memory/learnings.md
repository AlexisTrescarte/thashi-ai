...
### 2026-04-23T20:05:00Z — RULE-ADJUSTMENT
**Context**: Run `market-close` du 23 avril 2026 (15:05 CT). `memory/equities/portfolio.md` n'avait pas de baseline explicite (`Baseline date: _(first live run date)_`). Prior runs 20-22/04 portaient un résidu BTCUSD déjà réattribué à l'agent crypto + aucune position equities ouverte, donc aucun close snapshot propre n'avait été posé. Aujourd'hui 1 position ouverte (GOOGL 7 @ $339.29), cash 97.56% — premier close snapshot significatif.
**Takeaway**: Baseline fixée au 2026-04-23 (J0) avec equity $97,379.63, SPY $688.15 (Alpaca close bid 20:00:02Z), QQQ $652.88 (Alpaca mid 652.85/652.92 à 20:05:35Z), bench index normalisé à 100. L'equity initial $97,379.63 est sous le starting capital $100,000 (paper) pour raisons historiques documentées (résidu BTCUSD réattribué, frais API). Toutes les perf cumulées futures sont calculées vs ce J0. Le bench 50% SPY + 50% QQQ est simple-price-index (pas de réinvestissement div) — convention retenue pour la simplicité ; divergence attendue ≤ 30bps/an par rapport à total-return.
**Action**: Mis à jour `memory/equities/portfolio.md` section Baseline (date + equity + SPY/QQQ closes + bench index 100.00) et section ATH tracking ($97,382.43 au 22/04 = last_equity d'aujourd'hui). Snapshot cumul perf = 0% par construction (J0). Day perf bot -0.003% (quasi-flat), bench ≈ -0.65% (SPY -0.40% + QQQ -0.90%), alpha day +0.65pp — tape risk-off intraday (oil +4% sur Iran/Hormuz, software crash NOW -17% IBM -8%), GOOGL résiste (-0.12%). Aucun last-call pre-earnings CUT requis (GOOGL time stop planifié 28/04 close, pas demain BMO).
**Agent**: equities
