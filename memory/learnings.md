...
### 2026-04-27T13:41:28Z — LESSON
**Context**: Run `market-open` du 27 avril 2026 (08:41 CT, 09:41 ET). Marché ouvert. Équité $97,418.69, cash $95,007.40 (97.5%), 1 position (GOOGL 7 @ $339.29, +1.51%, native trail 8% actif HWM $347.52 stop $319.72), 0 ordre additionnel. Aucun bloc pre-market frais pour 04-27 dans `research_log.md` — la dernière entrée pre-market date du 23/04. La routine pre-market du 27/04 (06:00 CT) n'a pas tourné (pas d'entrée dans `runs.log` après 04-24 daily-review). Pre-flight tous verts : pas d'auto-défense, pas de loss cap (last_equity $97,418.20 vs current $97,418.69 = flat), cash 97.5% ≫ 10%, 1 position ≪ 30, FOMC 28-29 dans 24-72h donc sizing déjà capé une notch (mais sans queue ça n'a pas d'impact).
**Takeaway**: Re-confirmation de la règle invoquée le 22/04 : sans bloc pre-market frais listant une BUY queue, le comportement correct est no-op au market-open. La règle "DO NOT create a new idea at the open" du forbidden-list prime sur l'activity floor. Deuxième occurrence du même symptôme en 5 jours de trading (22/04, 27/04) : la routine pre-market a manqué deux fois — schedule pre-market est instable. À surveiller au prochain pre-market réussi : ré-évaluer GEV/VRT (rejected 23/04 sur spread + FOMO) qui ont eu une semaine de tape post-gap pour se calmer.
**Action**: (1) Aucun ordre placé. (2) Aucune réconciliation de stop nécessaire (GOOGL trailing natif actif et fonctionnel — HWM s'est ratchet de $347.52, stop monté à $319.72, soit -8% du HWM courant). (3) Notification Telegram envoyée (politique unifiée v2 : mandatoire à chaque run). (4) Surface à l'utilisateur : la routine pre-market 04-27 ne s'est pas déclenchée — vérifier `next_run_at` du trigger pre-market sur claude.ai. Si le pattern continue, considérer redondance ou alerte schedule. (5) Carry-forward pour le prochain pre-market : re-scorer GEV / VRT post-tape stabilization, ré-asserter sortie GOOGL 28/04 close pré-earnings 29/04 AMC.
**Agent**: equities
