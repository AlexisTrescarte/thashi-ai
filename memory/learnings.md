...
### 2026-04-28T15:39:00Z — LESSON
**Context**: Intraday-scan 10:30 CT slot, **first 04-28 routine recorded** in runs.log (no 04-28 pre-market block, no 04-28 market-open line — although a market-open ran on a separate branch this morning since the GOOGL trailing stop was replaced by a market-on-close sell at 13:41:58Z = 08:41 CT). Trigger schedule still degraded vs spec; runs.log gap from 04-24T20:45 to 04-28T15:39 means: 04-27 Monday entirely missed (pre-market + market-open + 4 intraday-scans + market-close + daily-review = 9 routines), plus 04-28 pre-market missed. State at scan: equity $97,459.57, cash $95,007.40 (97.48%), 1 position (GOOGL +3.21%, MOC sell 33e1dd7d queued for today's close), day P&L flat. Macro: QQQ flat near ATH, VIX subdued, no regime shift. FOMC decision tomorrow 13:00 CT (~95% hold), GOOGL earnings tomorrow AMC.
**Takeaway**: The 04-24 [INCIDENT] action item ("bind dated exits to GTD orders at fill, not future routines") **was applied this morning** by the 04-28 market-open branch — the trailing stop was canceled and replaced by a `tif=cls` market-on-close sell. This is the right move: even if every remaining 04-28 routine fails to wake up, GOOGL exits at 16:00 ET via the exchange. Validation of the 04-24 lesson on its first applicable test. Side-effect: `qty_available = 0` precludes a parallel intraday trailing stop (the MOC reserves the shares), so the position runs naked vs. intraday downside between now and close — acceptable trade given (a) +3.21% cushion, (b) ~5h to close, (c) certainty of pre-earnings exit > intraday stop optimization. No Pathway-B/C BUY today (no fresh dated catalyst surfaced + FOMC <24h imposes sizing-down per guardrails); Pathway A inactive (no 04-28 pre-market block).
**Action**: (1) Hold GOOGL — MOC sell will fire at 16:00 ET. (2) Watch the 11:30 / 12:30 / 13:30 / 14:30 slots for actual wake-up; if any miss again, this is now a multi-day [INCIDENT-2] cascade and should be surfaced loudly to the user at next opportunity (the 04-27 gap = 9 routines missed + 04-28 pre-market = 10 routines on top of the 04-24 cascade). (3) No new BUY today — discipline holds: FOMC + earnings binary tomorrow, sizing-down rule active, no fresh research surfacing today justifies an opportunistic open. (4) Next routine (11:30 intraday-scan) should re-verify the MOC sell is still active and the GOOGL price has not collapsed (a -5% intraday move, while not stop-protected, would still warrant a manual cut to avoid riding into the close at a worse mark).
**Agent**: equities
